...
public class AutoCannonWidget : CanvasWidget
{
	public AutoCannonWidget(ComponentPlayer componentPlayer, int slotIndex)
	{
		...
		// Solo el botón OK necesita traducción, Cancel y Help se dejan con X y ? respectivamente
		this.m_okButton.Text = LanguageControl.Get("AutoCannonWidget", "Accept");

		// Traducir los labels específicos del widget
	...
	{
		// Buscar y traducir labels específicos por su posición o contenido
		foreach (Widget child in this.Children)
		{
			if (child is LabelWidget labelWidget)
			{
				TranslateLabelText(labelWidget);
			}

			// Buscar en contenedores anidados
			if (child is CanvasWidget canvasWidget)
			{
				TranslateLabelsInCanvas(canvasWidget);
			}
		}
	}

	private void TranslateLabelsInCanvas(CanvasWidget canvas)
	{
		foreach (Widget child in canvas.Children)
		{
			...
			// Si hay más niveles anidados
			if (child is CanvasWidget nestedCanvas)
			{
				TranslateLabelsInCanvas(nestedCanvas);
			}
			else if (child is GridPanelWidget gridPanel)
			{
				TranslateLabelsInGridPanel(gridPanel);
			}
		}
	}

	private void TranslateLabelsInGridPanel(GridPanelWidget gridPanel)
	{
		foreach (Widget child in gridPanel.Children)
		{
			if (child is LabelWidget labelWidget)
			{
				TranslateLabelText(labelWidget);
	...
	private void TranslateLabelText(LabelWidget labelWidget)
	{
		string originalText = labelWidget.Text;

		switch (originalText)
		{
			case "Velocidad de proyectil":
				labelWidget.Text = LanguageControl.Get("AutoCannonWidget", "Velocidad De Proyectil");
				break;
			case "Disparos por segundo":
				labelWidget.Text = LanguageControl.Get("AutoCannonWidget", "Disparos Por Segundo");
				break;
			case "Dispersión de proyectiles":
				labelWidget.Text = LanguageControl.Get("AutoCannonWidget", "Dispersion De Proyectiles");
				break;
			case "Combustible":
				labelWidget.Text = LanguageControl.Get("AutoCannonWidget", "Combustible");
				break;
			case "Inventario":
				labelWidget.Text = LanguageControl.Get("AutoCannonWidget", "Inventario");
				break;
			case "Auto-Cañón":
				labelWidget.Text = LanguageControl.Get("AutoCannonWidget", "AutoCanon");
				break;
		}
	}
	...
	{
		if (this.m_speedSlider.IsSliding)
		{
			this.m_speedTextBox.Text = this.m_speedSlider.Value.ToString("0.##");
		}

		if (this.m_rateSlider.IsSliding)
		{
			this.m_rateTextBox.Text = this.m_rateSlider.Value.ToString("0.##");
		}

		if (this.m_spreadSlider.IsSliding)
		{
			this.m_spreadTextBox.Text = this.m_spreadSlider.Value.ToString("0.##");
		}

		if (this.m_speedTextBox.HasFocus)
		{
			float num;
			if (float.TryParse(this.m_speedTextBox.Text, out num))
			{
				this.m_speedSlider.Value = MathUtils.Clamp(num, this.m_speedSlider.MinValue, this.m_speedSlider.MaxValue);
			}
		}

		if (this.m_rateTextBox.HasFocus)
		{
			float num2;
			if (float.TryParse(this.m_rateTextBox.Text, out num2))
			{
				this.m_rateSlider.Value = MathUtils.Clamp(num2, this.m_rateSlider.MinValue, this.m_rateSlider.MaxValue);
			}
		}

		if (this.m_spreadTextBox.HasFocus)
		{
			float num3;
			if (float.TryParse(this.m_spreadTextBox.Text, out num3))
			{
		...
		if (slotCount != this.m_lastFuelCount)
		{
		...
		{
			string title = LanguageControl.Get("AutoCannonWidget", "AutoCannonHelpTitle");
			string message = LanguageControl.Get("AutoCannonWidget", "AutoCannonHelpMessage");
			string okText = LanguageControl.Get("AutoCannonWidget", "OK");

			DialogsManager.ShowDialog(this.m_componentPlayer.GuiWidget,
				new MessageDialog(title, message, okText, null, null));
		}
	}
